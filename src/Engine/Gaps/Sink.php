 ...
 * it, what it asks for, what it was made for, the reason it gives and its side, together with what the gap
 * holds. The fixer of a pass reports and fixes, the survey of the whitespace fuzz records.
 * @internal
 */
interface Sink
{
	/**
	 * @param array{Rule, Line, Node|Token, ?string, string} $claim
	...
	/** @param array{Rule, Space, Node|Token, ?string, string} $claim */
	public function space(array $claim, Token $previous, Token $token, string $found): void;

	/**
	 * @param array{Rule, int|array{int, ?int}, Node|Token, ?string, string} $claim  the claim the count violates, when it does
