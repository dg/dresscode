...
final class PassRunner
{
	...
	private function account(string $name, RuleContext $context, int $before, bool $checkSilent = true): void
	{
		...
		foreach ($reports as $i => $report) {
			...
			$denied = !$report->fixable || $report->silenced || $refused; // no fix, a comment or the risk
			...
			if ($denied && ($reports[$i + 1]->revision ?? $after) > $report->revision) {
				$this->violateContract($report->fixable
					? "Rule $name mutated the file after a suppressed report."
					: "Rule $name mutated the file after a report without a fix.");
			}
