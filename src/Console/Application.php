...
final class Application
{
	...
	private const Help = <<<'XX'
		Usage:
		  ...
		  dresscode init [options]               write dresscode.neon from how the code of the project is written
		...
		Options:
		  ...
		  --preset <name>...        add a preset; for init the standard to write, per by default
		  --rule <spec>...          enable or disable a rule: name=on or name=off
		  --only <name>...          run only these of the rules the configuration comes to, a preset
		                            standing for all of its rules
		  --stdin <path>            read the code from stdin as if it were the file at the path;
		                            fix writes the result to stdout
		  --skip-excluded           skip a named file that excludePaths of the configuration leaves out,
		                            which a hook or an editor naming every file it touches wants
	...
	private function runCheckOrFix(array $args, bool $fix): int
	{
		...
		if (is_string($args['--worker'])) { // the parent keeps the cache; the baseline decides what a rule may fix
			$worker = $args['--generate-baseline'] ? $config->baseline(null) : $config;
			$runner = $factory->createRunner($worker, $root, strict: (bool) $args['--strict-rules'], cache: false);
		...
		$generate = (bool) $args['--generate-baseline'];
		if ($generate && $fix) {
			throw new UsageException('The baseline is generated by check, not by fix.');
		} elseif ($generate && $args['--only']) { // it would forget what every other rule found
		...
		if ($generate) {
			return $this->generateBaseline($factory, $config, $root, $configFile, $files, $workers, $format);
		}
	 ...
	 * or into the default one beside the configuration, which the user then has to name to make it apply.
	 * @param  list<string>  $files
	 */
	...
	{
		$configured = $config->getBaseline();
		$name = $configured ?? self::defaultBaselineName($configFile);
		$file = RunnerFactory::toAbsolutePath($name, $root);
		$runner = $factory->createRunner($config->baseline(null), $root, cache: false);
	...
	{
		$name = $configFile === null ? '' : preg_replace('~\.dist$~', '', $configFile);
		return 'dresscode-baseline.' . (strtolower(pathinfo((string) $name, PATHINFO_EXTENSION)) === 'php' ? 'php' : 'neon');
	}
