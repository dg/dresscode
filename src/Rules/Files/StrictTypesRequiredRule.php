...
use PhpSyntax\{Parser, Token, Trivia, TriviaKind};
...
final class StrictTypesRequiredRule extends NodeRule implements ConfigurableRule
{
	...
	{
		return array_find($declare->items->getItems(), fn(DeclareItemNode $item) => strtolower($item->name->token->text) === 'strict_types');
	}
	...
	private function insert(int $index, Token $token, Trivia $tag, RuleContext $context): void
	{
		...
		$statement->setEdgeTrivia([new Trivia(TriviaKind::OpenTag, $text)], [new Trivia(TriviaKind::EndOfLine, $eol)]);
		...
		$context->getFile()->statements->insert($index, $statement);
	}
