...
final class NoInvisibleCharactersRule extends NodeRule
{
	...
	public function enter(Node|Token $node, RuleContext $context): void
	{
		...
		switch ($node->kind) {
			...
			case TokenKind::EncapsedAndWhitespace:
				...
				if ($context->report($node, "Invisible character $found in a string", fixable: !$nowdoc) && !$nowdoc) {
			...
			case TokenKind::StringVariableName:
	...
	{
		$characters = implode('|', array_map(fn(string $character) => preg_quote($character, '~'), array_keys(self::Characters)));
