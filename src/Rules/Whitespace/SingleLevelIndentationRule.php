...
use PhpSyntax\{Indentation, Node, Token, Trivia, TriviaKind};
...
final class SingleLevelIndentationRule extends NodeRule implements ConfigurableRule
{
		...
		foreach ($token->leadingTrivia as $trivia) {
			if ($trivia->isEndOfLine()) {
				...
				if (
					...
					&& ($this->singleLineComments || str_contains($trivia->text, "\n"))
				) {
	...
	{
		$unit = Indentation::width($style->indent, $style->toPhpSyntax());
		$width = Indentation::width($indentation, $style->toPhpSyntax());
