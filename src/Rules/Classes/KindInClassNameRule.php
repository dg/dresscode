	...
	public function enter(Node|Token $node, RuleContext $context): void
	{
		...
		if ($this->kind === self::Required) {
			...
			foreach ($words as $word) {
				...
				if (!$carries) {
					$context->report($name, "The class name must carry the word '$word'", fixable: false);
				}
			}
		...
		foreach ($words as $word) {
			...
			if (self::hasPrefix($text, $word)) {
				$context->report($name, "Useless prefix '" . substr($text, 0, $length) . "' in the class name", fixable: false);
			}

			if (self::hasSuffix($text, $word)) {
				$context->report($name, "Useless suffix '" . substr($text, -$length) . "' in the class name", fixable: false);
			}
		}

		if ($node instanceof ClassNode && !$node->modifiers->isAbstract() && self::hasSuffix($text, 'Error')) {
			$context->report($name, "Useless suffix '" . substr($text, -5) . "' in the class name", fixable: false);
		}
	}
