	...
	public function enter(Node|Token $node, RuleContext $context): void
	{
		...
		foreach ($node->find(NameNode::class) as $name) {
			...
			if (
				...
				|| !($parent instanceof StaticMethodCallNode || $parent instanceof ClassConstantFetchNode || $parent instanceof StaticPropertyFetchNode || $parent instanceof NewNode || $parent instanceof InstanceofNode)
				|| $name->findAncestor(ClassLikeNode::class) !== $node
				|| !$context->report($name, "The current class '$own' must be referenced as 'self'", risky: $parent instanceof StaticMethodCallNode)
			) {
				continue;
			}
