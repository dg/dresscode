 ...
 * The rule reports them and writes nothing: `__sleep()` returns the names of the properties to keep and
 ...
 * require it. Writing an empty `__unserialize()` instead would be wrong, since a class that has it gets no property
 * restored by PHP. A comment in or above the method keeps it, as it says why the method is there.
 */
#[RuleInfo(
...
final class NoSleepAndWakeupRule extends NodeRule
{
	...
	public function enter(Node|Token $node, RuleContext $context): void
	{
		...
		if ($this->isRemovable($node, $context)) {
			if ($context->report($node->name, "Method {$node->name->text}() is deprecated since PHP 8.5 and this one does nothing")) {
