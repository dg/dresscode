...
/**
 * A method whose whole body passes its parameters on to the parent method of the same name, `return parent::load($id)`,
 * says nothing the parent does not, and goes. The types decide whether a caller could tell the two apart, which
 * the method alone does not show: the parent must declare it with the same visibility, staticness, parameters and
 * return type (`Types::hasParentSignature()`).
 *
 * The method stays where it says more than its code: a doc comment or an attribute other than `#[\Override]`, `final`,
 * a trait the class uses, whose method of the name would take the place of the parent's, and a call that discards the
 * return value of a method that is not void. A body that passes the parameters in another order, or not all of them,
 * is not a repetition either.
 *
...
final class UselessOverridingMethodRule extends NodeRule
{
	...
	public function enter(Node|Token $node, RuleContext $context): void
	{
		...
		if (
			...
			|| !$context->report($node->name, "The method {$node->name->text}() only calls the parent method and says nothing more")
		) {
			return;
		}

		NodeHelpers::removeBetweenGaps($node, $context->getStyle()->eol);
	}
