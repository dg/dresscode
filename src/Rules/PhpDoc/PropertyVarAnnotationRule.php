...
/**
 * A property doc comment with a second `@var`, or with its `@var` below another annotation, is reported.
 */
#[RuleInfo(
...
final class PropertyVarAnnotationRule extends NodeRule
{
	...
	public function enter(Node|Token $node, RuleContext $context): void
	{
		...
		if (count($vars) > 1) {
			$context->report($node, 'Only one @var annotation is allowed in a property doc comment', trivia: $comment, fixable: false);
		} elseif ($vars && strtolower($tags[0]->name) !== '@var') {
			$context->report($node, 'The @var annotation must be the first one in a property doc comment', trivia: $comment, fixable: false);
		}
	}
}
