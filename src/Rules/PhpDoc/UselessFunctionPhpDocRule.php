...
final class UselessFunctionPhpDocRule extends NodeRule implements ConfigurableRule
{
	/** @var list<string> */
	...
	private function repeats(PhpDocTypeNode $annotation, ?TypeNode $native, NameResolver $resolver): bool
	{
		...
		if ($bare instanceof NamedTypeNode) {
			$traversable = NativeType::isTraversable($bare->name->text, $this->traversableTypeHints, fn(string $name) => $resolver->resolveClass($bare->name));
