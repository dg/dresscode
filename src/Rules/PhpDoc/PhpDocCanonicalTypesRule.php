...
use PHPStan\PhpDocParser\Ast\Type\{ArrayShapeItemNode, ArrayTypeNode, GenericTypeNode, IdentifierTypeNode, ObjectShapeItemNode, UnionTypeNode};
...
final class PhpDocCanonicalTypesRule extends NodeRule implements ConfigurableRule
{
	private const Short = [
		'boolean' => 'bool',
		'callback' => 'callable',
			...
			public function enterNode(PhpDocParserNode $node): PhpDocParserNode|int|null
			{
				...
				if ($node instanceof IdentifierTypeNode && !isset($this->keys[$node])) {
					...
					$canonical = $this->short[$lower]
						?? (in_array($lower, $this->builtin, true) ? $lower : null);
