...
final class CombinedIssetsRule extends NodeRule
{
	...
	private static function findPrecedingIsset(Node $left): ?IssetNode
	{
		return match (true) {
			...
			$left instanceof BinaryOpNode && $left->operator->is(TokenKind::BooleanAnd) && $left->right instanceof IssetNode => $left->right,
			default => null,
		};
	}
}
