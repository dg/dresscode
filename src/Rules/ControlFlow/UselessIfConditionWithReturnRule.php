 ...
 * comment is inside, otherwise only reported: `!$x` negated is `$x`, which need not be a boolean.
 */
#[RuleInfo(
...
final class UselessIfConditionWithReturnRule extends NodeRule
{
	...
	public function enter(Node|Token $node, RuleContext $context): void
	{
		...
		if ($ifValue) {
			$expr = $node->condition->withoutEdgeTrivia();
		} else {
			$expr = NodeHelpers::negate($node->condition);
		}

		$fixable = !($node->getFirstToken()?->hasCommentUpTo($last) ?? true) && NodeHelpers::isBoolean($expr);
