...
final class NoAlternativeSyntaxRule extends NodeRule
{
	...
	public function enter(Node|Token $node, RuleContext $context): void
	{
		...
		if ($node instanceof SwitchNode) {
			...
			$end = $node->endKeyword;
			if ($end === null || $node->colon === null || $node->statements === null) {
				return;
			}

			$block = $this->buildBlock($node->colon, $node->statements, $end->leadingTrivia, ($closeTag === null ? $semicolon ?? $end : $end)->trailingTrivia);
			$node->colon = null;
