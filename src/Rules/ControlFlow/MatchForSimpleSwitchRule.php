 ...
 *
 * Every fix is risky: a switch compares loosely and a match strictly, so a case of `0` catches the string
 * `'a'` today and an arm of `0` does not.
 */
#[RuleInfo(
...
final class MatchForSimpleSwitchRule extends NodeRule
{
	...
	private static function readArms(Statement\SwitchNode $switch): ?array
	{
		...
		if ($cases === [] || !$last instanceof CaseNode || $last->value !== null) {
			return null; // a switch without a default leaves the subject unanswered, a match raises an error
		}
