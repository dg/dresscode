 ...
 * bracket keeps the closing one where it is too. An array of several items on one line wider than maxWidth is
 * spread in either shape, and with keep more than ten items fill its lines up to the line length instead. Whatever
 * the frame, the opening bracket stays on the line of the code before it (an assignment, a return, a double arrow)
 * and each comma on the line of its item, and any other array kept on one line is left alone. Where the lines stand
...
final class MultiLineArrayRule extends GapRule implements ConfigurableRule
{
	...
	/** an array spread for its width with more items than this lets them fill its lines with keep, instead of a line each */
	private const MaxItemsOnOwnLines = 10;
	...
	public static function getOptionsSchema(): Schema
	{
		return Expect::structure([
			...
			'maxWidth' => Expect::int()->min(1)->nullable()
				->description('An array of several items on one line wider than this, from its opening bracket to its closing one, is spread over lines, more than ten items filling each line up to the line length with keep; null leaves the width to the author'),
		]);
	}
	...
	private function decide(Gap $gap, ?Node $array, bool $frame = false): ?array
	{
		...
		$decision = $gap->once($array, fn() => $this->decideNow($array, $gap->style));
	 ...
	 * @return ?array{bool, list<Claim>, Claim, Claim, Claim}
	 */
	private function decideNow(ArrayNode $array, Style $style): ?array
	{
	...
	/**
	 * The claims before the items of an array spread for its width with more than ten items: an item follows the one
	 * before on its line while the line fits into the line length, and begins the next one when it does not; null
	 * for an array whose items stand one on a line, as one with a comment does.
	 * @return ?list<Claim>
	 */
