 ...
 * of PHP at run time, is dresscode/name-fallback's: this rule neither qualifies it nor adds an import that would take
 * it over, and leaves a name dresscode/name-fallback writes bare alone. An import is not added where its name is
 * taken; one the markup of the file leaves no line for, or one a bare name would be taken over by unless
 * dresscode/name-fallback qualifies it, is reported to be written by hand. A global function or constant written with
 * the leading backslash loses the import nothing uses any more; the import of a class is left to
 * dresscode/unused-imports, which reads doc comments.
 */
#[RuleInfo(
	...
	private function fixGlobalNames(NamespaceNode $node, RuleContext $context): void
	{
		...
		foreach ([SymbolKind::ClassLike, SymbolKind::Function, SymbolKind::Constant] as $kind) {
			foreach ($uses[$kind->name] ?? [] as $key => $occurrences) {
				...
				// the import would take over a bare name of the symbol, which is dresscode/name-fallback's to decide: where
