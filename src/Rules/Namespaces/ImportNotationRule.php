 ...
 * would join another one. The order of the imports is the matter of dresscode/ordered-imports.
 */
#[RuleInfo(
...
final class ImportNotationRule extends NodeRule implements ConfigurableRule
{
	...
	public static function getOptionsSchema(): Schema
	{
		...
		return Expect::structure([
			'classes' => (clone $shape)->description('single gives every class its own import, combined puts all classes of the namespace into one; keep leaves them alone'),
	...
	public function enter(Node|Token $node, RuleContext $context): void
	{
		...
		foreach ($stmts->getItems() as $stmt) {
			if ($stmt instanceof UseNode && $stmt->isGroup() && $this->groupUse === 'expand' && $this->isChecked($stmt)) {
	...
	{
		return array_any($node->items->getItems(), fn(UseItemNode $item) => $this->shapes[self::kindOf($item->kind)] !== null);
	}
	...
	private function combine(array $uses, string $kind, RuleContext $context): void
	{
		...
		foreach (array_slice($uses, 1) as $use) {
			...
			// and a leading backslash belongs to dresscode/no-leading-backslash-in-import
