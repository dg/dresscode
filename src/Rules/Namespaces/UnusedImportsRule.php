...
final class UnusedImportsRule extends NodeRule implements ConfigurableRule
{
	...
	public static function getOptionsSchema(): Schema
	{
		return Expect::structure([
			'searchAnnotations' => Expect::bool(true)->description('A class name referenced in a doc comment counts as used, the name of an annotation such as @ORM\Entity included'),
		]);
	}
	...
	private function collectUsages(FileNode|NamespaceNode $scope, RuleContext $context): array
	{
		...
		foreach ($scope->find(NameNode::class) as $name) {
			...
			$parts = $name->parts;
			$kind = count($parts) > 1 ? self::Classes : self::kindOf($name->role);
	...
	private static function identifiers(PhpDocAstNode $node): array
	{
		...
		if ($node instanceof IdentifierTypeNode) {
			...
			// an annotation such as `@ORM\Entity`, `@Route` or `@Index` nested in another, unlike `@param` or `@phpstan-return`
