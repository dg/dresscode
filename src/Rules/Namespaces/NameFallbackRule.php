 ...
 * from the general to the particular: `functions` and `constants` speak of every global function and constant,
 * `optimizedFunctions` of the functions the compiler or opcache treats specially where a call of them is one PHP optimizes
 * with its arguments, `strlen()` with any, `dirname()` or `defined()` only with constant ones and `in_array()` with
 * a constant array, and `optimizedConstants` of the
 * constants PHP declares where the compiler computes with them: in a constant expression, such as `ENT_QUOTES | ENT_HTML5`
 * or `PHP_VERSION_ID >= 80400`, as a constant condition or as an operand of a logical operator, which opcache prunes, and
 * not where one is only passed or compared with a variable, where its value saves no more than the fetch. A value is
 * qualified, fallback, keep or, for the general keys, a map from a name or a pattern with `*` to one, and
 * `optimizedConstants` takes qualified or keep; the most particular answer decides, as in name-notation.
 *
 * A name is qualified in the shape name-notation gives it, with an import where that rule gives none: through an import
 * of the symbol under another name where the file has one, else with an import of its own, and with the leading
 * backslash where the name is taken; an import name-notation asks for and the markup of the file leaves no line for is
 * reported to be written by hand. A name is made bare only where the bare name reaches the global
 * symbol, and the import nothing needs any more goes with it. Every fix is risky unless the resolution of names is
 * certain, because the namespace may declare a function or a constant of that name elsewhere. A file without
...
final class NameFallbackRule extends NodeRule implements ConfigurableRule
{
	...
	/**
	 * Qualifies the bare names of the global symbol in the shape name-notation gives, an import where it gives none:
	 * through an import of the symbol under another name, with an import of its own, which qualifies them all at once,
	 * or each with the leading backslash where the name is taken. An import name-notation asks for and the markup leaves
	 * no line for is reported to be written by hand.
	 * @param  list<array{NameNode, string, string}>  $occurrences
