 ...
 * is: the code does not say that `[$obj, 'run']` is a callable at all, and without the types it cannot.
 */
#[RuleInfo(
...
final class FirstClassCallableNotationRule extends NodeRule
{
	...
	public function enter(Node|Token $node, RuleContext $context): void
	{
		...
		if (
			$callable === null
			|| !$context->report($node, "The callable must be written as $callable(...)", risky: $risky)
		) {
			return;
		}
	...
	private function readFromCallable(Expression\StaticMethodCallNode $call, RuleContext $context): ?string
	{
		...
		if ($value instanceof StringNode) {
			...
			return preg_match('~^\\\\?\w+(\\\\\w+)*(::\w+)?$~D', $value->value) === 1 ? '\\' . ltrim($value->value, '\\') : null;
		}
