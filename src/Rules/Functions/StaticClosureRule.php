...
final class StaticClosureRule extends NodeRule
{
	...
	private static function mayUseThis(Expression\ClosureNode|Expression\ArrowFunctionNode $closure): bool
	{
		foreach ($closure->find(Node::class) as $node) {
			$class = match (true) {
				$node instanceof Expression\StaticMethodCallNode, $node instanceof Expression\StaticPropertyFetchNode, $node instanceof Expression\ClassConstantFetchNode => $node->class,
	...
	private static function isBound(Expression\ClosureNode|Expression\ArrowFunctionNode $closure): bool
	{
		...
		return $call instanceof Expression\StaticMethodCallNode
			...
			&& preg_match('~(^|\\\)closure$~i', $call->class->text)
			&& $call->name instanceof IdentifierNode
