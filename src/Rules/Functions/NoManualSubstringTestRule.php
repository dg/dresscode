 ...
 * whose every byte of a character is one the encoding cannot begin a character with.
 */
#[RuleInfo(
...
final class NoManualSubstringTestRule extends NodeRule
{
	...
	): ?array
	{
		...
		// the mb_ functions count characters, str_contains() bytes, and an encoding whose trailing byte may
		// look like a character of its own answers otherwise; which encoding is in force the code does not say
	...
	): ?array
	{
		...
		if (count($arguments) === 3 && self::isZero($arguments[1])) {
			...
			// for an empty needle the comparison asks whether the haystack is empty, str_ends_with() says yes always
			return match (true) {
