 ...
 * parameters already begin on a line of their own has every parameter on a line of its own, each comma on the line
 * of its parameter and the closing parenthesis on the next; where they stand is the matter of dresscode/indentation.
 */
#[RuleInfo(
...
final class MultiLineSignatureRule extends GapRule implements ConfigurableRule
{
	...
	/** The width of the line depends on the tab width of the style, the widest one allowed on its line length. */
	private function reasonToSplit(FunctionNode|MethodNode $node, Style $style): ?string
	{
		...
		return $style->lineLength !== null && $width > $style->lineLength ? "the line is $width characters long" : null;
	}
}
