 ...
 *
 * The body must end with a `throw` or an `exit` and hold no `return` and no `yield` of its own, which is what
 * makes it certain: whatever stands before the last statement either falls through to it or leaves by
 * a return, and the return is not there. A magic method keeps its signature, PHP prescribing it.
 *
