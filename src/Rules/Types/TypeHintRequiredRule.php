		...
		foreach ($node->parameters->getItems() as $param) {
			...
			if ($native === null && $annotation === null) {
				if ($name !== null && !isset($prefixed[$name])) {
					$context->report($param->variable, "Parameter $name has no type hint nor @param annotation");
				}
			...
			if ($native === null) {
				$native = NativeType::fromAnnotation($annotation, NativeType::Parameter, $context->getPhpVersion(), $this->traversableTypeHints, $templates, fn(string $class) => $resolver->resolveClass((new Parser)->parseName($class), $node));
				if (
					$native === null
			...
			$bare = ltrim($native, '?');
			$traversable = NativeType::isTraversable($bare, $this->traversableTypeHints, fn(string $class) => $resolver->resolveClass((new Parser)->parseName($class), $node));
			if ($tag === null) {
				if ($traversable && ($name === null || !isset($prefixed[$name]))) {
					$context->report($param->variable, "Traversable parameter $name has no @param annotation saying what the items are");
				}
