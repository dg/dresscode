...
final class NativeType
{
	...
	{
		return in_array(strtolower($hint), self::Iterable, true)
			|| (
				preg_match('~^\\\?[A-Za-z_\x80-\xff][\w\x80-\xff\\\]*$~', $hint) === 1
				&& !in_array(strtolower($hint), self::Builtin, true)
				&& in_array(strtolower(ltrim($resolveClass($hint), '\\')), $traversableTypeHints, true)
			);
	}
