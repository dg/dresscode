 ...
 * by classes, members and functions that exist. A replacement the file replaces in its turn is followed to its end,
 ...
 * sentences, none when the file is sound.
 */
final class UpgradingTester
{
	 ...
	 * @return list<string>
	 */
	public static function check(string $file, string $root): array
	{
		...
		try {
			...
			if ($whole === null) {
				return ["The package $source is about is not installed in $root, so nothing of the file can be checked."];
			}
		...
		[$problems] = self::validate($all->profile->rules);
		return $problems === [] ? self::checkExistence(self::validate($reached->profile->rules)[1]) : $problems;
	}
	...
	private static function checkExistence(array $rules): array
	{
		...
		foreach ($classes as $old => $new) {
			...
			if (strtolower($new) === $old || isset($classes[strtolower($new)])) {
				...
				$problems[] = "replaced-classes: $written[$old], which does not exist" . ($new === $classes[$old] ? '.' : ", nor does $new it leads to.");
			}
		}
		...
		foreach ($members as $member => [$class, $name, $kind]) {
			...
			if ($next === $member || isset($members[$next])) {
				...
				$problems[] = "replaced-members: $written[$member], which does not exist" . ($first === "$class::$name" ? '.' : ", nor does $class::$name it leads to.");
			}
		}
