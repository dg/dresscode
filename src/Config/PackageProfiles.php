...
/**
 * What the installed packages bring to a project without being named in its configuration: the upgrading files a
 * package ships under `extra.dresscode.upgrading` of its composer.json, what the versions of a library retired and
 * what to write instead, as the options of the rules of DressCode in sections `since <version>`, of which the ones
 * the version the project stands on reaches apply (ProjectPackages::findVersion()), a later one having the last
 * word on an entry; and the extension a package names under `extra.dresscode.extension`. The root package takes
 * part too, and its own file applies whole. Such a file never turns a rule on: it speaks only for a rule the
 * project runs.
 * @internal
 */
	...
	public static function discover(ProjectPackages $project): self
	{
		...
		foreach ($packages as [$name, $path, $extra]) {
			...
			foreach ((array) ($extra['upgrading'] ?? []) as $file) {
				if (!is_string($file)) {
					throw new ConfigurationException("Package $name names something that is not a file name in extra.dresscode.upgrading of its composer.json.");
				} elseif ($path === null) {
