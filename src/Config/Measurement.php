...
final readonly class Measurement
{
	...
	public function __construct(
		...
		/** files the rule failed in under some value, left out of the places */
		public int $failed = 0,
	) {
	}
	...
	public function describe(): string
	{
		...
		return match (true) {
			...
			$parts === [] => "none of the $this->opportunities $this->unit agrees with any value",
			default => implode(', ', $parts) . " of $this->opportunities $this->unit"
				. ($this->neither ? ", $this->neither in none of them" : ''),
		} . ($this->failed ? sprintf(', %d failing %s left out', $this->failed, $this->failed === 1 ? 'file' : 'files') : '');
	}
