...
final class RunnerFactory
{
	...
	/**
	 * The lowest version the constraint of require.php allows, read only from the shapes that say it: alternatives
	 * joined by `||` or `|`, each of them `^V`, `~V`, `>=V` with or without an upper bound, or `V - V`. A constraint
	 * of any other shape leaves the version to the default rather than to a guess.
	 */
	public static function detectPhpVersion(?string $composerFile): ?string
	{
		...
		foreach (preg_split('~\s*\|\|?\s*~', trim($constraint)) ?: [] as $alternative) {
			if (!preg_match('~^(?|[\^\~](\d+(?:\.\d+)*)|>=\s*(\d+(?:\.\d+)*)(?:\s*,?\s*<=?\s*\d+(?:\.\d+)*)?|(\d+(?:\.\d+)*)\s*-\s*\d+(?:\.\d+)*)$~D', $alternative, $m)) {
				return null;
			}

			$version = implode('.', array_slice([...explode('.', $m[1]), '0'], 0, 2));
			$lowest = $lowest === null || version_compare($version, $lowest, '<') ? $version : $lowest;
		}
