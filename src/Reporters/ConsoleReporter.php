...
final class ConsoleReporter implements Reporter
{
	 ...
	 * own, unless the two differ in severity, when it must not hide behind a state that is not its own.
	 ...
	 * text can tell; a check says how many a fix would leave.
	 */
	private function formatVerdict(RunResult $result): string
	{
		...
		$parts = array_filter([
			...
			$result->countRiskyDeferred()
				? $result->countRiskyDeferred() . ' of them risky (' . implode(', ', array_map(self::formatRule(...), $result->listRiskyDeferredRules()))
					. '), fixed with --fix-risky or once their rules are named in fixRisky'
				: null,
		]);
