...
use DressCode\{FileResult, Helpers, Reporter, RunResult, Severity};
use function sprintf, strlen;


/**
 * Workflow commands of GitHub Actions: every violation becomes an annotation shown at its line
 * in the pull request. A fix run annotates what the fixed text still violates, at its lines in
 * that text.
 */
final class GithubReporter implements Reporter
{
	...
	public function reportFile(FileResult $result): void
	{
		...
		foreach ($this->fix ? $result->remaining : $result->violations as $violation) {
			$this->annotate(
	...
	public function finish(RunResult $result): void
	{
		...
		$violations = $result->countRemaining();
		$this->write(sprintf(
