	...
	public static function getTranslations(): array
	{
		return [
			...
			'attribute_empty_parentheses' => fn(array $o, Translation $t) => ($o['use_parentheses'] ?? false)
				...
				: $t->enable('dresscode/useless-attribute-parentheses'),
			...
			'fully_qualified_strict_types' => function (array $o, Translation $t) {
				...
				$t->enable('dresscode/name-notation', ['classes' => 'import', 'globalClasses' => 'keep']);
			},
			...
			'global_namespace_import' => function (array $o, Translation $t) {
				...
				$t->enable('dresscode/name-notation', [
					'globalClasses' => $shape(array_key_exists('import_classes', $o) ? $o['import_classes'] : true),
					'globalFunctions' => ['*' => $shape(array_key_exists('import_functions', $o) ? $o['import_functions'] : false)],
					'globalConstants' => ['*' => $shape(array_key_exists('import_constants', $o) ? $o['import_constants'] : false)],
				]);
			},
			// the fixer groups whatever a namespace has, so two names are enough, and it keeps the group it made
			'group_import' => fn(array $o, Translation $t) => $t->enable('dresscode/group-import', ['minImports' => 2])
				->enable('dresscode/import-notation', ['groupUse' => 'keep']),
			...
			'modernize_strpos' => function (array $o, Translation $t) {
				...
				$t->enable('dresscode/no-manual-substring-test');
			},
			...
			'native_function_invocation' => function (array $o, Translation $t) {
				...
				foreach ($o['include'] ?? ['@compiler_optimized'] as $name) {
					if ($name === '@compiler_optimized') {
						$options['optimizedFunctions'] = 'qualified';
				...
				$t->enable('dresscode/name-notation', ['globalFunctions' => ['*' => 'backslash']]);
				$t->enable('dresscode/name-fallback', $options + ($functions === [] ? [] : ['functions' => $functions]));
			},
			'native_constant_invocation' => function (array $o, Translation $t) {
				...
				$options = ($o['fix_built_in'] ?? true) ? ['optimizedConstants' => 'qualified'] : [];
				$constants = [];
				...
				// the fixer qualifies with the backslash
				$t->enable('dresscode/name-notation', ['globalConstants' => ['*' => 'backslash']]);
				if ($options !== [] || $constants !== []) {
					$t->enable('dresscode/name-fallback', $options + ($constants === [] ? [] : ['constants' => $constants]));
				}
			},
			'no_alias_functions' => function (array $o, Translation $t) {
				...
				$t->enable('dresscode/no-alias-functions', ($o['sets'] ?? null) === null ? [] : ['sets' => $sets]);
			},
			...
			'no_singleline_whitespace_before_semicolons' => fn(array $o, Translation $t) => $t->enable('dresscode/semicolon-spacing', ['allowOwnLine' => true]),
			...
			'phpdoc_readonly_class_comment_to_keyword' => 'dresscode/readonly-for-annotation',
			...
			'regular_callable_call' => 'dresscode/no-call-user-func',
			...
			'self_static_accessor' => fn(array $o, Translation $t) => $t->enable('dresscode/self-for-current-class', ['onStatic' => true]),
			...
			'single_space_around_construct' => fn(array $o, Translation $t) => $t->enable('dresscode/construct-spacing', ['allowMultiLineExpression' => true]),
			...
			'static_private_method' => 'dresscode/static-for-private-method-without-this',
