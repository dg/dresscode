...
test('every slug is built in the shape of its kind', function () use ($rules) {
	...
	$exceptions = [
		...
		'full-opening-tag', 'line-ending', 'nowdoc-without-interpolation',
		'phpdoc-null-last', 'phpdoc-trim', 'property-phpdoc-single-line', 'property-var-annotation',
		'reference-throwable-only', 'static-closure', 'strict-call',
		'strict-comparison', 'switch-case-colon', 'symbolic-logical-operators',
	...
	$shapeOf = function (string $slug, string $class) use ($suffixes, $prefixes, $infixes, $verbs): ?string {
		...
		if (str_starts_with($slug, 'forbidden-') || str_starts_with($slug, 'replaced-')) {
			return 'policy';
		}
