...
/**
 * Random damage to the indentation, the blank lines, the line breaks and the spaces of the corpus, under PER with
 * every spacing rule of the catalogue on and exact. The fixed file is the reference, not the original one: the corpus is foreign code that no
 * preset accepts, while its fixed form is a fixpoint the fixer must return to from any whitespace around it.
 */

use DressCode\{Analyses, Config, Line, Style};
use DressCode\Config\{PresetResolver, RuleRegistry};
use DressCode\Engine\{Diff, FileProcessor};
use DressCode\Engine\Gaps\Resolver;
use DressCode\Presets\Per;
use DressCode\Testing\GapSurvey;
use PhpSyntax\Nodes\{ArrayItemNode, ElseIfNode, FileNode, MatchArmNode};
use PhpSyntax\Nodes\Expression\{AssignmentNode, CombinedAssignmentNode, ShellExecNode};
...
use PhpSyntax\{Parser, Token, TokenKind};
...
function candidateGaps(Resolver $gaps, FileNode $file, Style $style): array
{
	...
	foreach (GapSurvey::collect($gaps, $file, $style) as $gap) {
		...
		if ($gap[0] === 'blank') {
			...
			$gap[0] === 'line'
			&& $gap[2] === Line::Same
			&& !isShapedByAuthor($gap[1])
			&& $gap[1]->getLineWidth($style->toPhpSyntax()) <= $style->lineLength
		) {
			// a line already wider than the line length may be broken where a join would widen it
			$splits[] = $offset;
		}
	}
...
/**
 * Whether the token stands in the condition of an if, elseif, while or do-while, or in a property hook, or next to an
 * assignment or a `=>` after something spanning several lines: split over lines, such a construct is laid out line by
 * line, and such an operator may begin the next line, as the author's choice, so a split there is not a damage
 * anything undoes.
 */
function isShapedByAuthor(Token $token): bool
{
	foreach ([$token, $token->getPrevious()] as $operator) {
		...
		if (
			...
			&& $owner->getFirstToken()?->getLine() !== $operator->getPrevious()?->getLine()
		) {
			return true;
		}
	}

	for ($node = $token->parent; $node?->parent !== null; $node = $node->parent) {
		$parent = $node->parent;
		if (
			$node instanceof PropertyHookNode
			|| (($parent instanceof IfNode || $parent instanceof ElseIfNode || $parent instanceof WhileNode || $parent instanceof DoWhileNode)
...
function candidateSpaces(FileNode $file): array
{
	...
	for ($token = $file->getFirstToken(); $token !== null; $token = $token->getNext()) {
		...
		// printed, so a space put there is output and not layout; what follows `<?=` is the template's
		if (
			$space === null
			|| $next === null
			|| $offset === null
			|| insideString($token)
			|| insideString($next)
			|| $token->is(TokenKind::InlineHtml, TokenKind::CloseTag, TokenKind::HaltCompilerData, TokenKind::OpenTagWithEcho)
			|| $next->is(TokenKind::InlineHtml, TokenKind::CloseTag, TokenKind::HaltCompilerData)
		) {
			continue;
		}

		// closed, the two must stay the same two tokens: `- -` welds into `--`, and `. 4.0` splits anew as `.4` `.0`
		$texts = array_map(fn($t) => is_array($t) ? $t[1] : $t, array_slice(@token_get_all("<?php {$token->text}{$next->text}"), 1));
		$welds = $texts !== [$token->text, $next->text];
...
$enabled = [
	...
	'dresscode/blank-lines' => [
		'betweenDeclarations' => 2, 'betweenMethods' => 2, 'betweenMethodsInInterface' => 1, 'beforeFirstMethod' => 0, 'afterLastMethod' => 0,
...
foreach (array_keys($registry->getRules()) as $name) {
	if (str_ends_with($name, '-spacing')) {
		$enabled[$name] = match ($name) {
			...
			'dresscode/semicolon-spacing' => ['after' => 'single'],
...
$rules = $resolver->build($resolver->resolve(new Config(presets: [Per::class], rules: $enabled), Config::DefaultPhpVersion));
...
// DRESSCODE_FUZZ_ONLY=<regex> keeps the files whose path matches, with the same damage as in the whole run
if ($only = getenv('DRESSCODE_FUZZ_ONLY')) {
	$files = array_values(array_filter($files, fn(string $file) => preg_match("~$only~", $file) === 1));
}
