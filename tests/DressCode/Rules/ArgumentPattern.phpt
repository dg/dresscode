...
use DressCode\Rules\Classes\ArgumentPattern;
use DressCode\Rules\Classes\ArgumentPatternItem;
...
test('a pattern is read as the arguments of a call', function () {
	Assert::equal(
		[
			...
			new ArgumentPatternItem('f', parameterName: 'fallback'),
			new ArgumentPatternItem(literal: [false], parameterName: 'strict'),
			new ArgumentPatternItem(variadic: true),
		],
		ArgumentPattern::parse('$name, TRUE, null, -1.5, "x", [], fallback: $f, strict: false, ...')->items,
	);
...
test('a named item takes only the argument passed by that name, with the parameters or without them', function () {
	Assert::same(['f' => 'fallback: $cb', '...' => ['$key']], bind('fallback: $f, ...', 'f($key, fallback: $cb)'));
	Assert::null(bind('fallback: $f, ...', 'f($key, $cb)', ['key', 'fallback']));
	Assert::null(bind('fallback: $f, ...', 'f($key, generator: $cb)'));
