...
test('the plugin rule is known by name', function () {
	...
	[$code, $out] = runPlugin([
		'check',
		'--rule', 'acme/no-var-dump=off',
		'--rule', 'dresscode/ordered-imports=off',
		'--rule', 'dresscode/no-trailing-whitespace=off',
	]);
	Assert::same(0, $code);
...
test('a default configuration stands in for a missing file', function () {
	...
	[$code, $out, $err] = runPlugin(['check'], $root, new Config(extensions: [Extension::class], presets: ['acme/default'], paths: ['src']));
	Assert::same('', $err);
	Assert::same(1, $code);
