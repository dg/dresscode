...
function violation(string $rule, string $message, string $line, int $occurrence = 1): Violation
{
	...
	return new Violation($rule, $message, 1, null, Severity::Error, fingerprint: Violation::createFingerprint($rule, $message, $content, $occurrence));
}
...
test('a fingerprint of nothing but digits survives the round trip', function () use ($dir) {
	...
	Assert::same([1, 0], [$baseline->countMatched(), $baseline->countUnused(['src/a.php' => null])]);
});
...
test('knows the violations of a file and counts the matched and the unused entries', function () use ($file, $a, $b, $c) {
	...
	Assert::same(2, $baseline->countUnused($scope));
	...
	Assert::same(1, $baseline->countUnused($scope));
	Assert::same(0, $baseline->countUnused(['src/a.php' => null])); // a file the run did not process says nothing
	Assert::same(0, $baseline->countUnused(['src/b.php' => ['test/a']])); // nor does a rule that did not run
	Assert::same(1, $baseline->countUnused(['src/b.php' => ['test/b']]));
});
