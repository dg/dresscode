...
use DressCode\Config\{Loader, Proposal, RunnerFactory, Sample};
...
test('init lists what the namespaces declare from every file, and proposes the preset of an installed framework', function () {
	...
	[$code, $out] = runInit($root, ['--preset', 'per']);
	Assert::same(0, $code);
	Assert::contains('Namespaces 3 functions and 3 constants declared, resolved for certain, symfony-configurator for symfony/dependency-injection', $out);
	Assert::match(<<<'XX'
		%A%presets:
			- per
			- symfony-configurator  # symfony/dependency-injection is installed
		%A%
	...
	Assert::same('dresscode/symfony-configurator', $resolved->namespacedFunctions['Symfony\Component\DependencyInjection\Loader\Configurator\service']);
	Assert::true($resolved->getRule('dresscode/no-unlisted-namespaced-declaration')?->isActive());
});
