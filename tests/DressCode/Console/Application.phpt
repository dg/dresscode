...
test('--only narrows the run to what it names, in the workers and in the cache too', function () use ($root) {
	...
	Assert::same(1, $code);
	Assert::match('%A%FOUND  4 violations, 4 of them fixable in 4 files%A%', $expected);
	...
	Assert::same(1, $code);
	Assert::match('%A%FOUND  4 violations, 4 of them fixable in 4 files%A%', $out);
	...
	Assert::match('%A%Not running%A%  dresscode/eof-newline %s%left out by --only%A%', $out);
