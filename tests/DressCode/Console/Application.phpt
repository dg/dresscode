...
test('a baseline is generated by check over what a fix leaves and silences what it knows', function () use ($root) {
	...
	Assert::same(2, $code);
	Assert::same("Error: The baseline is generated over code a fix leaves alone: a fix would change 1 file, 1 failed. Run fix first.\n  src/a.php\n  src/c.php\n", $err);
	...
	[$code, $out] = runApp($root, ['check', '--config', "$root/baseline.php"]);
	Assert::same(0, $code);
	Assert::match("%A%Warning: 1 entry of the baseline no longer matches a violation; regenerate it\n\nOK  1 violation in the baseline in 2 files\n", $out);
...
test('migrate-suppressions narrows a directory holding the configured paths to them', function () use ($root) {
	@mkdir("$root/other"); // @ - may exist
