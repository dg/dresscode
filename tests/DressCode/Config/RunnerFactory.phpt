...
test('the lowest version the constraint of require.php allows', function () use ($fixtures) {
	...
	// a constraint whose lower bound cannot be read says nothing, not the first number it names
	Assert::null($detect('{"require": {"php": "<8.4"}}'));
	Assert::null($detect('{"require": {"php": ">5.6"}}'));
	Assert::null($detect('{"require": {"php": "!=8.1 >=8.0"}}'));
