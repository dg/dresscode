...
test('what the rules refuse is said with the rule, in whatever section it stands', function () {
	...
	Assert::match('The package lib.neon is about is not installed in %a%', check("package: acme/other\n\nsince 1.0:\n\treplaced-classes: []\n")[0]);
});
...
test('a replacement that does not exist at the installed version, even at the end of what it leads to, and a circle are problems', function () {
	Assert::same(
		[
			...
			'replaced-classes: Acme\Lib\IOldest is replaced by Acme\Lib\IOlder, which does not exist, nor does Acme\Lib\Missing it leads to.',
			...
			'replaced-members: Acme\Lib\Form::OLDEST is replaced by Older, which does not exist, nor does Acme\Lib\Form::Missing it leads to.',
